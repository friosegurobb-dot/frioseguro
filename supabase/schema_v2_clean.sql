-- ============================================================================
-- SCHEMA SUPABASE v2.0 - Sistema Monitoreo Reefer
-- VERSIÓN LIMPIA - Borra todo y recrea
-- ============================================================================
-- IMPORTANTE: Esto borra todas las tablas existentes!
-- Ejecutar en SQL Editor de Supabase
-- ============================================================================

-- Borrar tablas existentes (en orden inverso por dependencias)
DROP TABLE IF EXISTS daily_stats CASCADE;
DROP TABLE IF EXISTS config_history CASCADE;
DROP TABLE IF EXISTS defrost_sessions CASCADE;
DROP TABLE IF EXISTS commands CASCADE;
DROP TABLE IF EXISTS maintenance_logs CASCADE;
DROP TABLE IF EXISTS door_events CASCADE;
DROP TABLE IF EXISTS power_events CASCADE;
DROP TABLE IF EXISTS alerts CASCADE;
DROP TABLE IF EXISTS readings CASCADE;
DROP TABLE IF EXISTS devices CASCADE;

-- Borrar vistas
DROP VIEW IF EXISTS v_devices_status CASCADE;
DROP VIEW IF EXISTS v_active_alerts CASCADE;
DROP VIEW IF EXISTS v_latest_readings CASCADE;
DROP VIEW IF EXISTS v_today_summary CASCADE;

-- Borrar funciones
DROP FUNCTION IF EXISTS get_device_history CASCADE;
DROP FUNCTION IF EXISTS calculate_daily_stats CASCADE;
DROP FUNCTION IF EXISTS update_device_online CASCADE;
DROP FUNCTION IF EXISTS update_updated_at CASCADE;
DROP FUNCTION IF EXISTS update_device_last_seen CASCADE;

-- ============================================================================
-- 1. TABLA DEVICES
-- ============================================================================
CREATE TABLE devices (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    device_id VARCHAR(50) UNIQUE NOT NULL,
    name VARCHAR(100) NOT NULL,
    location VARCHAR(200),
    location_lat DECIMAL(10, 7),
    location_lon DECIMAL(10, 7),
    temp_max DECIMAL(5,2) DEFAULT -15.0,
    temp_critical DECIMAL(5,2) DEFAULT -10.0,
    alert_delay_sec INTEGER DEFAULT 300,
    door_open_max_sec INTEGER DEFAULT 120,
    defrost_cooldown_sec INTEGER DEFAULT 1800,
    is_online BOOLEAN DEFAULT FALSE,
    last_seen_at TIMESTAMPTZ,
    firmware_version VARCHAR(20),
    wifi_rssi INTEGER,
    ip_address VARCHAR(50),
    telegram_enabled BOOLEAN DEFAULT TRUE,
    supabase_enabled BOOLEAN DEFAULT TRUE,
    sim800_enabled BOOLEAN DEFAULT FALSE,
    power_monitor_enabled BOOLEAN DEFAULT FALSE,
    current_sensor_enabled BOOLEAN DEFAULT FALSE,
    multi_door_enabled BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_devices_device_id ON devices(device_id);
CREATE INDEX idx_devices_online ON devices(is_online);

-- ============================================================================
-- 2. TABLA READINGS
-- ============================================================================
CREATE TABLE readings (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    temp1 DECIMAL(5,2),
    temp2 DECIMAL(5,2),
    temp3 DECIMAL(5,2),
    temp4 DECIMAL(5,2),
    temp_avg DECIMAL(5,2),
    temp_dht DECIMAL(5,2),
    humidity DECIMAL(5,2),
    door1_open BOOLEAN DEFAULT FALSE,
    door2_open BOOLEAN DEFAULT FALSE,
    door3_open BOOLEAN DEFAULT FALSE,
    door4_open BOOLEAN DEFAULT FALSE,
    ac_power BOOLEAN DEFAULT TRUE,
    battery_voltage DECIMAL(4,2),
    battery_percent INTEGER,
    current_amps DECIMAL(6,2),
    compressor_running BOOLEAN,
    relay_on BOOLEAN DEFAULT FALSE,
    buzzer_on BOOLEAN DEFAULT FALSE,
    alert_active BOOLEAN DEFAULT FALSE,
    defrost_mode BOOLEAN DEFAULT FALSE,
    simulation_mode BOOLEAN DEFAULT FALSE,
    wifi_rssi INTEGER,
    gsm_signal INTEGER,
    uptime_sec BIGINT,
    free_heap INTEGER,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_readings_device ON readings(device_id);
CREATE INDEX idx_readings_created ON readings(created_at DESC);
CREATE INDEX idx_readings_device_date ON readings(device_id, created_at DESC);

-- ============================================================================
-- 3. TABLA ALERTS
-- ============================================================================
CREATE TABLE alerts (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    alert_type VARCHAR(50) NOT NULL,
    severity VARCHAR(20) NOT NULL,
    message TEXT,
    temperature DECIMAL(5,2),
    current_amps DECIMAL(6,2),
    door_open_seconds INTEGER,
    acknowledged BOOLEAN DEFAULT FALSE,
    acknowledged_at TIMESTAMPTZ,
    acknowledged_by VARCHAR(100),
    resolved BOOLEAN DEFAULT FALSE,
    resolved_at TIMESTAMPTZ,
    resolution_notes TEXT,
    telegram_sent BOOLEAN DEFAULT FALSE,
    sms_sent BOOLEAN DEFAULT FALSE,
    push_sent BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_alerts_device ON alerts(device_id);
CREATE INDEX idx_alerts_type ON alerts(alert_type);
CREATE INDEX idx_alerts_severity ON alerts(severity);
CREATE INDEX idx_alerts_active ON alerts(resolved) WHERE resolved = FALSE;
CREATE INDEX idx_alerts_date ON alerts(created_at DESC);

-- ============================================================================
-- 4. TABLA POWER_EVENTS
-- ============================================================================
CREATE TABLE power_events (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    event_type VARCHAR(20) NOT NULL,
    outage_duration_sec INTEGER,
    battery_used_percent INTEGER,
    min_battery_voltage DECIMAL(4,2),
    sms_sent BOOLEAN DEFAULT FALSE,
    telegram_sent BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_power_device ON power_events(device_id);
CREATE INDEX idx_power_date ON power_events(created_at DESC);

-- ============================================================================
-- 5. TABLA DOOR_EVENTS
-- ============================================================================
CREATE TABLE door_events (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    door_number INTEGER NOT NULL DEFAULT 1,
    door_name VARCHAR(50),
    event_type VARCHAR(20) NOT NULL,
    open_duration_sec INTEGER,
    temp_at_open DECIMAL(5,2),
    temp_at_close DECIMAL(5,2),
    temp_rise DECIMAL(5,2),
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_door_device ON door_events(device_id);
CREATE INDEX idx_door_number ON door_events(door_number);
CREATE INDEX idx_door_date ON door_events(created_at DESC);

-- ============================================================================
-- 6. TABLA MAINTENANCE_LOGS
-- ============================================================================
CREATE TABLE maintenance_logs (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    maintenance_type VARCHAR(50) NOT NULL,
    compressor_hours DECIMAL(10,1),
    compressor_starts INTEGER,
    max_current_ever DECIMAL(6,2),
    notes TEXT,
    performed_by VARCHAR(100),
    next_maintenance_at TIMESTAMPTZ,
    next_maintenance_hours DECIMAL(10,1),
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_maintenance_device ON maintenance_logs(device_id);
CREATE INDEX idx_maintenance_type ON maintenance_logs(maintenance_type);

-- ============================================================================
-- 7. TABLA COMMANDS
-- ============================================================================
CREATE TABLE commands (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    command VARCHAR(50) NOT NULL,
    parameters JSONB,
    status VARCHAR(20) DEFAULT 'pending',
    sent_at TIMESTAMPTZ,
    executed_at TIMESTAMPTZ,
    result TEXT,
    source VARCHAR(50),
    created_by VARCHAR(100),
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_commands_device ON commands(device_id);
CREATE INDEX idx_commands_pending ON commands(status) WHERE status = 'pending';

-- ============================================================================
-- 8. TABLA CONFIG_HISTORY
-- ============================================================================
CREATE TABLE config_history (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    config_key VARCHAR(50) NOT NULL,
    old_value TEXT,
    new_value TEXT,
    changed_by VARCHAR(100),
    change_source VARCHAR(50),
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_config_device ON config_history(device_id);
CREATE INDEX idx_config_date ON config_history(created_at DESC);

-- ============================================================================
-- 9. TABLA DEFROST_SESSIONS
-- ============================================================================
CREATE TABLE defrost_sessions (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    started_at TIMESTAMPTZ NOT NULL,
    ended_at TIMESTAMPTZ,
    duration_minutes INTEGER,
    temp_at_start DECIMAL(5,2),
    temp_at_end DECIMAL(5,2),
    temp_max_during DECIMAL(5,2),
    triggered_by VARCHAR(50),
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_defrost_device ON defrost_sessions(device_id);
CREATE INDEX idx_defrost_date ON defrost_sessions(created_at DESC);

-- ============================================================================
-- 10. TABLA DAILY_STATS
-- ============================================================================
CREATE TABLE daily_stats (
    id BIGSERIAL PRIMARY KEY,
    device_id VARCHAR(50) NOT NULL,
    stats_date DATE NOT NULL,
    temp_avg DECIMAL(5,2),
    temp_min DECIMAL(5,2),
    temp_max DECIMAL(5,2),
    temp_readings_count INTEGER,
    alerts_count INTEGER DEFAULT 0,
    critical_alerts_count INTEGER DEFAULT 0,
    door_opens_count INTEGER DEFAULT 0,
    door_total_open_minutes INTEGER DEFAULT 0,
    power_outages_count INTEGER DEFAULT 0,
    power_outage_total_minutes INTEGER DEFAULT 0,
    compressor_hours DECIMAL(5,2),
    compressor_starts INTEGER,
    avg_current DECIMAL(6,2),
    max_current DECIMAL(6,2),
    online_percent DECIMAL(5,2),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(device_id, stats_date)
);

CREATE INDEX idx_daily_device ON daily_stats(device_id);
CREATE INDEX idx_daily_date ON daily_stats(stats_date DESC);

-- ============================================================================
-- VISTAS
-- ============================================================================

CREATE VIEW v_devices_status AS
SELECT 
    d.device_id,
    d.name,
    d.location,
    d.is_online,
    d.last_seen_at,
    d.wifi_rssi,
    r.temp_avg AS current_temp,
    r.humidity AS current_humidity,
    r.door1_open,
    r.ac_power,
    r.alert_active,
    r.defrost_mode,
    r.compressor_running,
    r.current_amps,
    EXTRACT(EPOCH FROM (NOW() - d.last_seen_at)) AS seconds_since_seen
FROM devices d
LEFT JOIN LATERAL (
    SELECT * FROM readings 
    WHERE device_id = d.device_id 
    ORDER BY created_at DESC 
    LIMIT 1
) r ON true;

CREATE VIEW v_active_alerts AS
SELECT 
    a.*,
    d.name AS device_name,
    d.location
FROM alerts a
JOIN devices d ON d.device_id = a.device_id
WHERE a.resolved = FALSE
ORDER BY 
    CASE a.severity 
        WHEN 'emergency' THEN 1 
        WHEN 'critical' THEN 2 
        WHEN 'warning' THEN 3 
        ELSE 4 
    END,
    a.created_at DESC;

CREATE VIEW v_latest_readings AS
SELECT DISTINCT ON (device_id) *
FROM readings
ORDER BY device_id, created_at DESC;

CREATE VIEW v_today_summary AS
SELECT 
    device_id,
    COUNT(*) AS readings_count,
    AVG(temp_avg) AS avg_temp,
    MIN(temp_avg) AS min_temp,
    MAX(temp_avg) AS max_temp,
    COUNT(*) FILTER (WHERE alert_active) AS alert_readings,
    COUNT(*) FILTER (WHERE door1_open) AS door_open_readings,
    COUNT(*) FILTER (WHERE NOT ac_power) AS power_out_readings
FROM readings
WHERE created_at >= CURRENT_DATE
GROUP BY device_id;

-- ============================================================================
-- FUNCIONES
-- ============================================================================

CREATE FUNCTION get_device_history(p_device_id VARCHAR, p_hours INTEGER DEFAULT 24)
RETURNS TABLE (
    reading_time TIMESTAMPTZ,
    temp_avg DECIMAL,
    humidity DECIMAL,
    door_open BOOLEAN,
    ac_power BOOLEAN,
    alert_active BOOLEAN,
    current_amps DECIMAL
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        r.created_at AS reading_time,
        r.temp_avg,
        r.humidity,
        r.door1_open,
        r.ac_power,
        r.alert_active,
        r.current_amps
    FROM readings r
    WHERE r.device_id = p_device_id
    AND r.created_at > NOW() - (p_hours || ' hours')::INTERVAL
    ORDER BY r.created_at DESC
    LIMIT 10000;
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION update_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION update_device_last_seen()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE devices 
    SET last_seen_at = NOW(), is_online = TRUE
    WHERE device_id = NEW.device_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- ============================================================================
-- TRIGGERS
-- ============================================================================

CREATE TRIGGER tr_devices_updated
    BEFORE UPDATE ON devices
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER tr_readings_update_device
    AFTER INSERT ON readings
    FOR EACH ROW
    EXECUTE FUNCTION update_device_last_seen();

-- ============================================================================
-- RLS (Row Level Security)
-- ============================================================================

ALTER TABLE devices ENABLE ROW LEVEL SECURITY;
ALTER TABLE readings ENABLE ROW LEVEL SECURITY;
ALTER TABLE alerts ENABLE ROW LEVEL SECURITY;
ALTER TABLE power_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE door_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE commands ENABLE ROW LEVEL SECURITY;
ALTER TABLE maintenance_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE config_history ENABLE ROW LEVEL SECURITY;
ALTER TABLE defrost_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE daily_stats ENABLE ROW LEVEL SECURITY;

-- Políticas para acceso anónimo (API key)
CREATE POLICY "allow_all_devices" ON devices FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_readings" ON readings FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_alerts" ON alerts FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_power" ON power_events FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_door" ON door_events FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_commands" ON commands FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_maintenance" ON maintenance_logs FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_config" ON config_history FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_defrost" ON defrost_sessions FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "allow_all_daily" ON daily_stats FOR ALL USING (true) WITH CHECK (true);

-- ============================================================================
-- DATOS INICIALES
-- ============================================================================

INSERT INTO devices (device_id, name, location) VALUES
    ('REEFER-01', 'Reefer Principal', 'Campamento Base - Sector A'),
    ('REEFER-02', 'Reefer Carnes', 'Campamento Base - Sector A'),
    ('REEFER-03', 'Reefer Lácteos', 'Campamento Base - Sector B'),
    ('REEFER-04', 'Reefer Verduras', 'Campamento Base - Sector B'),
    ('REEFER-05', 'Reefer Bebidas', 'Campamento Base - Sector C');

-- ============================================================================
-- LISTO!
-- ============================================================================
